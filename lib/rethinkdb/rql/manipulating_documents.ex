defmodule Rethinkdb.Rql.ManipulatingDocument do
  ...
  defmacro __using__(_opts) do
    quote do
      ...
      def pluck(selector, %{} = query) do
        new_term(:'PLUCK', [selector], query)
      end

      def without(selector, %{} = query) do
        new_term(:'WITHOUT', [selector], query)
      end

      def merge(object, %{} = query) do
      ...
      def append(value, %{} = query) do
        new_term(:'APPEND', [value], query)
      end

      def prepend(value, %{} = query) do
        new_term(:'PREPEND', [value], query)
      end

      def difference(array, %{} = query) do
        new_term(:'DIFFERENCE', [array], query)
      end

      def set_insert(value, %{} = query) do
        new_term(:'SET_INSERT', [value], query)
      end

      def set_intersection(array, %{} = query) do
        new_term(:'SET_INTERSECTION', [array], query)
      end

      def set_difference(array, %{} = query) do
        new_term(:'SET_DIFFERENCE', [array], query)
      end

      def has_fields(selectors, %{} = query) do
        new_term(:'HAS_FIELDS', [selectors], query)
      end

      def insert_at(index, value, %{} = query) do
        new_term(:'INSERT_AT', [index, value], query)
      end

      def splice_at(index, array, %{} = query) do
        new_term(:'SPLICE_AT', [index, array], query)
      end

      def delete_at(index, endindex \\ nil, %{} = query) do
        new_term(:'DELETE_AT', [index | endindex && [endindex] || []], query)
      end

      def change_at(index, value, %{} = query) do
